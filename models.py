...
def create_modules(module_defs):
    ...
    for i, module_def in enumerate(module_defs):
        ...
        if module_def['type'] == 'convolutional':
            ...
            d = int(module_def['dilation'])
            #pad = (kernel_size - 1) // 2 if int(module_def['pad']==1) else 0
            if (int(module_def['pad'])==1):
                pad = (kernel_size - 1) // 2
            elif (int(module_def['pad'])==3):
                pad = 3
            elif (int(module_def['pad'])==2):
                pad = 2
            else:
                pad = 0
        ...
        elif module_def['type'] == 'deformconv':
            bn = int(module_def['batch_normalize'])
            filters = int(module_def['filters'])
            kernel_size = int(module_def['size'])
            #d = int(module_def['dilation'])
            ...
            if (int(module_def['pad'])==1):
                ...
                pad = 0
            modules.add_module('deformconv_%d' % i, DeformConv2d(inc=output_filters[-1],
                                                        outc=filters,
                                                        kernel_size=kernel_size,
                                                        stride=int(module_def['stride']),
                                                        padding=pad,
...
class Darknet(nn.Module):
    ...
    def forward(self, x, targets=None, var=0):
        ...
        for i, (module_def, module) in enumerate(zip(self.module_defs, self.module_list)):
            ...
            if mtype in ['convolutional', 'upsample', 'maxpool','deformconv']:
                x = module(x)
            elif mtype == 'route':
